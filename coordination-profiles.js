...
(function(){
  ...
  function filterDangerZones(zones){
    ...
    return zones.filter(z=>{
      ...
      // The candidate itself is the second transmitter, so one occupied
      // contributor means a 2TX IM3 product.
  ...
  window.intermods=function(occupied,maxOrder,relevantRange){
    const p=profile();
    const products=originalIntermods(
      occupied,
      Math.min(maxOrder||p.maxOrder,p.maxOrder),
      relevantRange
    );
  ...
  window.precomputeDangerZones=function(occupied,maxOrder,relevantRange){
    const p=profile();
    const zones=originalDangerZones(
      occupied,
      Math.min(maxOrder||p.maxOrder,p.maxOrder),
      relevantRange
    );
  ...
  async function bind(){
    ...
    window.CRF_COORDINATION_STATE.baseMinSeparation=
      Number.parseFloat(minInput.value)||0.8;
    ...
    minInput.addEventListener("input",function(){
      if(!window.CRF_COORDINATION_STATE.applying){
        ...
        if(Number.isFinite(v)&&v>0)
          window.CRF_COORDINATION_STATE.baseMinSeparation=v;
      }
    });
