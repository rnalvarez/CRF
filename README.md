...
**Nivel informativo separado para "fuera de rango" (no un descuento sobre el tier).** El ×0.15 anterior solo bajaba el score, pero el tier se decidía antes y podía ser crítico/advertencia igual — confirmado con casos reales. Ahora `classifyConflict()` recibe `inRange` y lo resuelve *antes* de decidir nada: fuera del rango relevante, el resultado es siempre `ℹ️ FUERA DE RANGO` (nivel aparte, no un nivel de riesgo), sin importar orden ni distancia — nunca puede convertirse en crítico o advertencia por sí solo. El producto se sigue calculando, mostrando en la tabla marcado "(fuera de rango)", y aporta un poco al score (`OUT_OF_RANGE_WEIGHT=0.15`, ahora aplicado dentro de la rama de clasificación, no como multiplicador externo después). En `TIER_RANK`, `fuera_de_rango` rankea apenas arriba de "recomendado" y abajo de "revisar", así que un hit real dentro de rango siempre gana la comparación de peor-tier. El margen de `relevantRange` (antes fijo en 2 MHz) ahora es el campo "Margen de análisis" en la sección 1, configurable — rango operativo real + margen = relevantRange, explícito y separado.
